...
light_cone_parameter_type
setup_light_cone(in light_t l)
{
  light_cone_parameter_type result = { 0.0, 0.0, 0.0, false };
...
light_accumulate(in    vec3   P, // fragment position (wc)
                 ...
                 inout float3 ambient,
                 inout float3 diffuse,
                 inout float3 specular)
{
  for (uint i = 0; i < length(light_list); ++i) {
    if (light_list[i].active) {
      float3 L           = light_vector     (light_list[i], P);
      ...
      if (0.0 < NdotL) {
        light_cone_parameter_type lcp = setup_light_cone(light_list[i]);
